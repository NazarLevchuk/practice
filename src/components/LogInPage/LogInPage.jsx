...
import {
  upDateInputTextActionCreator,
  userDataStorageActionCreator,
} from '../../redux/loggInReducer';
import s from './LogInPage.module.scss';

export function LogInPage({ setIsLoggedIn, dispatch, state }) {
  ...
  const handlerBlurEmail = () => {
    ...
    if (
      !re.test(String(loginRef.current.value).toLowerCase()) &&
      !rp.test(String(loginRef.current.value))
    ) {
      setButtonState(true);
  ...
  const onInputChange = () => {
    ...
    const emailText = loginRef.current.value;
  ...
  const ErrorBlock = React.useCallback(
      ...
      <div className={s.logInForm}>
        ...
        <form onSubmit={handleSubmit} className={s.form_container}>
          <div className={s.input_container}>
            <input
              value={state.loggInReducer.inputNameValue}
              ref={nameRef}
          ...
          <div className={s.input_container}>
            <input
              style={
                errorEmailBlockState
                  ? { border: 'solid black' }
                  : { border: 'solid red' }
              }
              value={state.loggInReducer.inputEmailValue}
              ref={loginRef}
              onBlur={handlerBlurEmail}
              onChange={onEmailBothCall}
              required
              name="email"
              type="text"
          ...
          <div className={s.input_container}>
            <input
              style={
                errorPasswordBlockState
                  ? { border: 'solid black' }
                  : { border: 'solid red' }
              }
              value={state.loggInReducer.inputPasswordValue}
              ref={passwordRef}
              onBlur={handlerBlurPass}
              onChange={onPasswordBothCall}
              required
              name="password"
              type="password"
